...
TOKEN_REQUIRED_OPTION_NAME = "shadaloo_emblems_required"
TOKEN_AVAILABLE_PERCENTAGE_OPTION_NAME = "shadaloo_emblems_available_percentage"
...
def get_required_token_count(multiworld: MultiWorld, player: int) -> int:
    return max(1, min(100, int(get_option_value(multiworld, player, TOKEN_REQUIRED_OPTION_NAME))))

def get_available_token_count(multiworld: MultiWorld, player: int) -> int:
    required_count = get_required_token_count(multiworld, player)
...
def get_enabled_item_count(world: World, multiworld: MultiWorld, player: int, goal_name: str) -> int:
    ...
    for name, item in world.item_name_to_item.items():
        ...
        if name == TOKEN_ITEM_NAME:
            enabled_item_count += get_available_token_count(multiworld, player) if is_token_goal(goal_name) else 0
            continue
...
def before_create_items_all(item_config: dict[str, int|dict], world: World, multiworld: MultiWorld, player: int) -> dict[str, int|dict]:
    ...
    if is_token_goal(goal_name):
        required_count = get_required_token_count(multiworld, player)
        available_count = get_available_token_count(multiworld, player)
